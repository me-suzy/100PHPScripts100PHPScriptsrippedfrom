...
if ((isset($HTTP_POST_VARS["MM_update"])) && ($HTTP_POST_VARS["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE url_table SET website=%s, active=%s, datechanged=%s WHERE id=%s",
                       GetSQLValueString($HTTP_POST_VARS['website'], "text"),
                       GetSQLValueString($HTTP_POST_VARS['active'], "text"),
                       GetSQLValueString($HTTP_POST_VARS['datechanged'], "date"),
                       GetSQLValueString($HTTP_POST_VARS['id'], "int"));

  mysql_select_db($database_dbConnect, $dbConnect);
  $Result1 = mysql_query($updateSQL, $dbConnect) or die(mysql_error());

  $updateGoTo = "urlListInd.php?id= ".$HTTP_POST_VARS['return'];
  //if (isset($HTTP_SERVER_VARS['QUERY_STRING'])) {
...
if ((isset($HTTP_POST_VARS["MM_update"])) && ($HTTP_POST_VARS["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE url_table SET website=%s, active=%s, datechanged=%s WHERE id=%s",
                       GetSQLValueString($HTTP_POST_VARS['website'], "text"),
                       GetSQLValueString($HTTP_POST_VARS['active'], "text"),
                       GetSQLValueString($HTTP_POST_VARS['datechanged'], "text"));
  mysql_select_db($database_dbConnect, $dbConnect);
  $Result1 = mysql_query($updateSQL, $dbConnect) or die(mysql_error());

  $updateGoTo = "mainTemplate.php?option=4";
  if (isset($HTTP_SERVER_VARS['QUERY_STRING'])) {
    $updateGoTo .= (strpos($updateGoTo, '?')) ? "&" : "?";
    $updateGoTo .= $HTTP_SERVER_VARS['QUERY_STRING'];
  }
        ...
        <tr> 
          ...
          <td width="61%" valign="top"> <form name="form1" method="POST" action="<?php echo $editFormAction; ?>">
              <table width="100%" border="0" cellpadding="2" cellspacing="0" class="supermenuNoshadow" >
                ...
                <tr> 
                  <td align="right" class="smallstatsText">URL number:</td>
                  <td class="smallstatsText"><?php echo $row_Recordset1['id']; ?></td>
                </tr>
                <tr> 
                  <td align="right" class="smallstatsText">website:</td>
                  <td class="smallstatsText"><input name="website" type="text" id="question2" value="<?php echo $row_Recordset1['website']; ?>" size="40" maxlength="254"> 
                  </td>
                </tr>
                <tr> 
                ...
                <tr> 
                  <td align="right" valign="top" class="smallstatsText">date changed:</td>
                  <td class="smallstatsText"> <input name="datechanged" type="text" id="datechanged" value="<?php echo $row_Recordset1['datechanged']; ?>" maxlength="10"> 
                  </td>
                </tr>
                <tr> 
                  <td align="right" valign="top" class="smallstatsText"><br> </td>
                  <td class="smallstatsText"><p> 
            ...
            <p><font color="#CCCCCC"><span class="smallstatsText"><a href="urlListInd.php?id=<?php echo $return; ?>">&lt;&lt; 
              back to user's urls</a><br>
