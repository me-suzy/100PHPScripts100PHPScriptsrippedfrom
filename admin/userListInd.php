...
$query_urlList = "SELECT user.username, url_table.userid, url_table.website, url_table.active FROM user INNER JOIN url_table ON user.id = url_table.userid where url_table.userid=".$id." ORDER BY username";
...
<table width="100%" border="0" cellspacing="0" cellpadding="2" class="supermenuNoshadow">
  <tr> 
    ...
    <td class="menuheader">website</td>
    <td class="menuheader">active?</td>
    <td width="5%" class="menuheaderAction">action</td>
  </tr>
  <?php do { ?>
  <tr> 
    <td class="smallstatsText"><?php echo $row_urlList['username']; ?></td>
    <td class="smallstatsText"><?php echo $row_urlList['website']; ?></td>
    <td class="smallstatsText"><?php echo $row_urlList['active']; ?></td>
    <td class="smalltext">options</td>
  </tr>
  <?php } while ($row_urlList = mysql_fetch_assoc($urlList)); ?>
  <tr> 
    <td colspan="4" align="center" ><a href="<?php printf("%s?pageNum_urlList=%d%s", $currentPage, 0, $queryString_urlList); ?>"><img src="images/first.gif" width="30" height="11" border="0"></a> 
